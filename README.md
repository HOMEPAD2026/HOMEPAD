...
One coin per capital, 197 capitals. A capital is claimed by launching on the **Hybrid factory** with name = country name, ticker = capital (or ISO3 when the capital is too long/non-ASCII), and logo = that country's canonical outline at `https://homepad.fun/world/logos/<ISO2>.svg`. The claim modal locks all three; the user's wallet signs, so the user is the on-chain creator and the existing 70% creator / 30% `$HOME` split applies with no new contracts.

The map's source of truth is the chain: `world.js` re-derives claims from `fetchAllLaunches()` (first Hybrid launch matching name + ticker + exact logo URL wins). Anyone can also claim through the normal launch form by matching those three fields — it's the same rule. `world-data.js` and `world/logos/` are generated by `worldgen/` (see its README; regenerating logos is a breaking change for existing claims since verification is an exact URL match).

**Firebase is optional.** `firebase-config.js` ships as `null` and the map works without it. Filling it in makes the site record each claim to Firestore (`claims/{iso2}`, create-only per `firestore.rules`) for the features that will need a database: wallet login, governance votes, city unlocks. The map never trusts Firestore over the factory.

Planned: once a capital passes a market-cap milestone, its country's other cities open to claim.
