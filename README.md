...
**Defend-or-lose window.** A fresh claim has `WORLD_RESET_GRACE_SEC` (1 hour) to reach `WORLD_RESET_MCAP_USD` ($30K) market cap. If it doesn't, anyone can reset the capital: pay `WORLD_RESET_FEE_HOMEPAD` (5,000 `$HOMEPAD`) to the `$HOME` treasury, then launch it again themselves. The map (`world.js`) verifies both halves on-chain — the fee transfer's block timestamp must fall between the old claim's grace deadline and the new claim's launch — before it will show a takeover as valid or let the claim modal offer one. The mcap-at-reset-time check is enforced by the app, not provable from past chain state alone; an airtight version needs a small registry contract, which is a natural next step once this is validated. All three constants live in `config.js`.
