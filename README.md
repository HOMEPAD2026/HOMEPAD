...
**Reset rule — purely market-cap gated, no time window.** Whenever a claimed capital's current market cap is under `WORLD_RESET_MCAP_USD` ($50K) — whether that's five minutes or five months after it launched — anyone can reset it: pay `WORLD_RESET_FEE_HOMEPAD` (2,000,000 `$HOMEPAD`) to the `$HOME` treasury, then launch it again themselves. The map (`world.js`) verifies this on-chain before showing a takeover as valid or letting the claim modal offer one: the fee transfer's block timestamp must fall after the current holder's own launch and at or before the new claim's launch. The mcap-at-reset-time check is enforced by the app, not provable from past chain state alone; an airtight version needs a small registry contract, which is a natural next step once this is validated. Both constants live in `config.js`.
