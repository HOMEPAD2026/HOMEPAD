/* snap-core.js — generated by tools/build-bundles.mjs from api/_snap-core.mjs. Do not edit; edit api/_snap-core.mjs. */
(function () {
  "use strict";
  ...
  const SNAP_BURN = ["0x0000000000000000000000000000000000000000", "0x000000000000000000000000000000000000dead", "0xdead000000000000000042069420694206942069"];
  ...
  async function stepJob(io, job, { chunk = 9000, wave = 1, until = null, onProgress = null } = {}) {
  ...
  async function extendJob(io, job, newHi, { chunk = 9000, wave = 1 } = {}) {
  ...
  async function readLocks(io, token, tag = "latest") {
  ...
  async function scanPositions(io, token, from = 1, { batch = 100, max = 3000, until = null } = {}) {
    token = lcs(token);
    const k = io.keccak;
  ...
  async function lpAt(io, positions, tag) {
    const k = io.keccak;
  ...
  function finishRows(job, base, { supplyNow, locked = null, lp = null } = {}) {
  ...
  const fingerprint = (csv, keccak) => keccak(asciiHexUtf8(csv));
  ...
  function merkleRoot(entries, keccak) {
  ...
  function diffLists(before, after, hold) {
  ...
  function lorenz(list, hold, points = 40) {
  ...
  const packRows = (rows) => rows.map((x) => [x.a.slice(2), b36(x.v), x.min === x.v ? "=" : b36(x.min), x.now === x.v ? "=" : b36(x.now), b36(x.locked || 0n), b36(x.lp || 0n)].join("|"));
  ...
  window.ArcSnapCore = { SNAP_VERSION, SNAP_ADDR, SNAP_BURN, units, parseUnits, pctOf, checksum, selector, newJob, stepJob, extendJob, readLocks, lockedByOwner, sqrtAtTick, amountsFor, scanPositions, lpAt, finishRows, applyFilters, calcAmounts, toCsv, fingerprint, merkleRoot, diffLists, lorenz, packRows, unpackRows, normFilters, sigText, filterKey };
})();
