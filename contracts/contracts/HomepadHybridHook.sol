...
/// @title HOMEPAD Hybrid Launch Hook
/// @notice The default launch mode. A real Uniswap v4 pool exists from the
///         same transaction the token is created in — visible on
///         Dexscreener and tradeable immediately — and gets its
///         curve-like, snipe-resistant price action from HOW that
///         liquidity is added, not from a custom pricing override: the
///         factory seeds the pool with a single-sided position (100% of
///         the sellable supply, 0% ETH) starting at the launch price, the
///         same technique real single-sided-liquidity launchpads use.
///         Since a concentrated position's own price impact grows the
///         more of it gets bought through, an early buyer with a lot of
///         ETH still can't acquire the whole supply "at the floor price"
///         the way a flat-price sale would let them — the price moves
///         against them continuously, same shape as a bonding curve,
///         using nothing but Uniswap's native liquidity math.
///
///         This hook's only job (identical to HomepadHook, the Instant
///         Liquidity mode's hook) is the live fee split: each pool's swap
///         fee is set to 0% at the Uniswap level, and this hook takes the
///         cut itself on every swap, paying creator/$HOME immediately —
///         no separate collect step.
contract HomepadHybridHook is BaseHook {
    struct HybridFeeConfig {
        address creator;
    ...
    mapping(PoolId => HybridFeeConfig) public hybridFeeConfigs;
    ...
    function registerHybridPool(PoolKey calldata key, HybridFeeConfig calldata cfg) external onlyFactory {
        hybridFeeConfigs[PoolIdLibrary.toId(key)] = cfg;
    }
        ...
        returns (bytes4, int128)
    {
        ...
        HybridFeeConfig memory cfg = hybridFeeConfigs[id];
    ...
    function _routeFee(PoolId id, Currency currency, uint256 fee, HybridFeeConfig memory cfg) internal {
