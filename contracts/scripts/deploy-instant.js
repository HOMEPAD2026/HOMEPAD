// Deploys the "instant launch" system: HomepadHook (at a CREATE2-mined
// address satisfying Uniswap V4's hook permission bits) + HomepadFactoryInstant.
//
// Required env vars:
...
//
// Optional (same meaning as scripts/deploy-v4.js):
...
// Must match HomepadHook.getHookPermissions(): afterSwap + afterSwapReturnDelta.
...
async function main() {
  ...
  console.log("2/5 — Mining a salt for HomepadHook's address (this can take a moment)...");
  const HomepadHookFactory = await ethers.getContractFactory("HomepadHook");
  // _admin is explicit (not msg.sender) since this deploys via Create2Deployer —
  // msg.sender in the hook's constructor would otherwise be that helper
  // contract's own address, not our actual deployer account.
  const deployTx = await HomepadHookFactory.getDeployTransaction(poolManagerAddr, deployerSigner.address);
  ...
  console.log("3/5 — Deploying HomepadHook via CREATE2...");
  const deployHookTx = await create2Deployer.deploy(salt, hookInitCode);
  await deployHookTx.wait();
  const hook = await ethers.getContractAt("HomepadHook", predictedHookAddr);
  ...
  console.log("     HomepadHook deployed to:", predictedHookAddr);

  console.log("4/5 — Deploying HomepadFactoryInstant and wiring it to the hook...");
  ...
  console.log("5/5 — Deploying HomepadSwapRouter...");
  ...
  await verifyIfPossible(hre, { name: "Instant Hook", address: predictedHookAddr, contract: "contracts/HomepadHook.sol:HomepadHook", constructorArgs: [poolManagerAddr, deployerSigner.address] });
  await verifyIfPossible(hre, { name: "Instant Liquidity Factory", address: factoryAddr, contract: "contracts/HomepadFactoryInstant.sol:HomepadFactoryInstant", constructorArgs: [homeTreasury, platformWallet, poolManagerAddr, predictedHookAddr, tickSpacing, baseFeeBps, creatorShareBps, homeShareBps] });
  await verifyIfPossible(hre, { name: "Instant Swap Router", address: routerAddr, contract: "contracts/HomepadSwapRouter.sol:HomepadSwapRouter", constructorArgs: [poolManagerAddr, predictedHookAddr, tickSpacing] });
